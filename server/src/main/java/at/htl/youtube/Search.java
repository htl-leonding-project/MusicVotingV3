...
public class Search {
	public List<Song> getSearchFromYoutube(String queryTerm) {
		...
		while (doc == null && attempts < 5) {
			try {
				doc = Jsoup.connect(baseUrl + queryTerm).header("User-Agent",
		...
		JsonArray videoArray = json.getJsonObject("contents").getJsonObject("twoColumnSearchResultsRenderer")
				...
				.getJsonObject(0).getJsonObject("itemSectionRenderer").getJsonArray("contents");
		...
		for (int i = 0; i < videoArray.size() - 1; i++) {
			...
			if (video != null) {
				...
				String thumbnail = video.getJsonObject("thumbnail").getJsonArray("thumbnails").getJsonObject(0)
						.getString("url");
	...
	private int convertStringToDuration(String duration) {
		...
		if (splitted.length == 3) {
			result += 60 * 60 * Integer.parseInt(splitted[0]) + 60 * Integer.parseInt(splitted[1]) + Integer.parseInt(
					splitted[2]);
